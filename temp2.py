...
usgs_path = 'C:/Temp/3DEP/3DEP_aspect_UTM.tif'
sfm_path = 'C:/Temp/3DEP/3DEP_aspect_UTM.tif'
#ls8_path = 'C:/Temp/0311/ls8/LC08_038029_20210311_albedo.tif'
usgs_offset = -13


usgs_source = gdal.Open(usgs_path, gdalconst.GA_ReadOnly)
sfm_source = gdal.Open(sfm_path, gdalconst.GA_ReadOnly)
...
out = gdal.Warp('C:/Temp/bareground/aspect/YC20200805_aspect_ext_UTM.tif', sfm_source, options = warp_options)
'''

warp2_options = gdal.WarpOptions(format = 'GTiff', outputBounds = [minX, minY, maxX, maxY], 
                                width = new_cols, height = new_rows,
                                srcSRS = usgs_proj, dstSRS = usgs_proj)
out2 = gdal.Warp('C:/Temp/3DEP/resampled/3DEP_ycoords_res_UTM.tif', usgs_source, options = warp2_options)
'''
...
'''
sfm_band = sfm_source.GetRasterBand(1)
sfm_arr = sfm_band.ReadAsArray()


sfm_ndv = sfm_band.GetNoDataValue()
usgs_ndv = usgs_source.GetRasterBand(1).GetNoDataValue()

sfm_arr[np.isnan(sfm_arr)] = sfm_ndv



print(np.max(sfm_arr))

usgs_band = usgs_source.GetRasterBand(1)
usgs_arr = usgs_band.ReadAsArray()
usgs_arr[usgs_arr==-999999] = sfm_ndv
#usgs_arr += usgs_offset

indices = np.where(sfm_arr!=sfm_ndv)
#indices = np.where(np.isnan(sfm_arr))
usgs_arr[indices] = sfm_arr[indices]
usgs_arr[usgs_arr == sfm_ndv] = np.nan
usgs_arr[usgs_arr == usgs_ndv] = np.nan
print(np.max(usgs_arr))


tcu.write_geotiff('C:/Temp/bareground/aspect/aspect.tif', np.shape(usgs_arr), new_geo, 32612, usgs_arr)
'''
sfm_source = None
usgs_source = None
out = None
